...
@ApiModel(description = "Конкретное достижение, полученное авторизованным студентом")
public interface AchievementReceivedView {

    /*  Данные полученного студентом достижения  */
    @ApiModelProperty(value = "id полученного достижения. Not null. >0", example = "4")
    @JsonProperty("receivedAchieveId")
    int getId();

    @ApiModelProperty(value = "Статус награды полученного достижения (получена награда или нет). Not null", example = "true")
    ...
    @Value("#{target.achievementForAchieveOfStudent.creatorOfAchieve}")
